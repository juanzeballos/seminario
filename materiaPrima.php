...
?>
	<!-- Encabezado -->
              ...
              <li class="nav-item  dropdown ">
         		...
         		<div class="dropdown-menu bg-dark menu-activo ">
        	    	...
        	    	<a class="dropdown-item text-secondary"  href="empleados.php">Empleado</a>
	       		   <a class="dropdown-item text-light active bg-secondary"  href="materiaPrima.php">Insumos</a>
		...
		<div class="row mb-4 mt-5">	
			...
			Fecha: <input type="month" id="fechaInsumos" name="fechaInsumos"  value="<?php echo date_format("d/m/Y")?>">
			...
			<?php
			include ('config.php');
			$registroInsumos=mysqli_query($conexion,"select nombreInsumo, montoInsumo,fechaInsumo from materia_prima");
			$totalInsumos=0;
			if (isset($_POST['fechaInsumos'])) 
			{
				?>
				<table class="table table-striped table-bordered">
					...
					<tbody >	
				<?php
				while ($reg=mysqli_fetch_array($registroInsumos))
				{
					$newDate = date("Y-m", strtotime($reg['fechaInsumo'])) ;

					if ($newDate == $_POST['fechaInsumos'] ) {
					
					$totalInsumos = $totalInsumos + $reg['montoInsumo'];
				
					?>
							
						
						<tr>
							<td><?php echo $reg['nombreInsumo'] ;?>
							</td>
						
							<td> <?php echo "$".$reg['montoInsumo'] ;?>						
							</td>

							<td><?php echo $fechaInsumos = date("d/m", strtotime($reg['fechaInsumo']));?> 
							</td>
						 </tr>
